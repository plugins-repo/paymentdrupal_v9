...
namespace Drupal\commerce_paymentdrupal_v9\Plugin\Commerce\PaymentGateway;

// require_once __DIR__ . '/../../../../vendor/autoload.php';
require_once __DIR__ . '/../../../../src/Controller/paymentdrupal_v9RedirectController.php';
...
use paymentdrupal_v9\paymentdrupal_v9Common\paymentdrupal_v9Common;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\RedirectResponse;

define('PF_SOFTWARE_NAME', 'drupalcommerce-2.x');
define('PF_SOFTWARE_VER', '2.32');
define('PF_MODULE_NAME', 'paymentdrupal_v9-drupalcommerce-2.x');
 ...
 *   id = "paymentdrupal_v9",
 *   label = "paymentdrupal_v9",
 *   display_label = "paymentdrupal_v9",
 *   forms = {
 *     "offsite-payment" = "Drupal\commerce_paymentdrupal_v9\PluginForm\OffsiteRedirect\PaymentOffsiteForm",
 *   },
...
class OffsiteRedirect extends OffsitePaymentGatewayBase
{
    ...
    public function onNotify(Request $request)
    {
        ...
        paymentdrupal_v9Common::pflog('paymentdrupal_v9 ITN call received');

        // Notify paymentdrupal_v9 that information has been received
        header('HTTP/1.0 200 OK');
        flush();

        // Get data sent by paymentdrupal_v9
        paymentdrupal_v9Common::pflog('Get posted data');

        // Posted variables from ITN
        $pfData = paymentdrupal_v9Common::pfGetData();

        paymentdrupal_v9Common::pflog('paymentdrupal_v9 Data: ' . print_r($pfData, true));

        if ($pfData === false) {
            $pfError  = true;
            $pfErrMsg = paymentdrupal_v9Common::PF_ERR_BAD_ACCESS;
        }

        // Verify security signature
        if (!$pfError) {
            paymentdrupal_v9Common::pflog('Verify security signature');
            ...
            if (!paymentdrupal_v9Common::pfValidSignature($pfData, $pfParamString, $passphrase)) {
                $pfError  = true;
                $pfErrMsg = paymentdrupal_v9Common::PF_ERR_INVALID_SIGNATURE;
            }
        }

        // Verify data received
        if (!$pfError) {
            paymentdrupal_v9Common::pflog('Verify data received');

            $pfValid = paymentdrupal_v9Common::pfValidData($pfHost, $pfParamString);
    ...
    public function processOrder($pfError, $pfData, $pfErrMsg): void
    {
        ...
        if (!$pfError) {
            paymentdrupal_v9Common::pflog('Check status and update order');
        ...
        if ($pfError) {
            paymentdrupal_v9Common::pflog('Error: ' . $pfErrMsg);
        }
    }
}
